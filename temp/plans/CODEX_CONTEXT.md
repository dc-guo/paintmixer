...
- Ring markers on the image show where each palette color came from; the active color's marker is larger. Manual hex colors have no marker.
- Markers are draggable (Coolors-style): dragging a marker re-samples that palette color live from the pixel under it.
- Clicking the artwork does NOT add to the palette — it previews the color in the inspector (dashed ring on the image, RGB/CMYK/viability in the Selected color card) with an explicit "Add to palette" action.
...
The detail page also shows the source artwork the palette was built from, stored as a downscaled local thumbnail (~480px JPEG) alongside the saved palette; gallery cards show the same thumbnail. Full-resolution artwork is never persisted (Guardrail 3); if localStorage quota is hit, thumbnails are dropped before color data.

Saved palettes are editable (2026-07-07): inline rename on the detail page, and an "Edit in workspace" action that reloads the palette (stored thumbnail as the artwork, colors with their marker positions) into the workspace, where Save becomes "Update palette" and writes back to the same palette. A fresh save also enters editing mode so repeated saves update rather than duplicate; uploading new artwork starts a fresh project.
...
12. (2026-07-07; URL corrected 2026-07-24) SHIPPED: PaintBridge is LIVE at https://dc-guo.github.io/paintbridge/ on GitHub Pages. (The repo was renamed `paintmixer` → `paintbridge`; the old `/paintmixer/` Pages URL is dead, and the build serves from `--base=/paintbridge/`.) The repo is public (Diane approved; no LICENSE file = all rights reserved; commits use her GitHub noreply email going forward). Deploys: merge to `main` runs `.github/workflows/deploy.yml` (tests + typecheck + build). PRs run `.github/workflows/ci.yml`, and `main` is branch-protected requiring the `test` check.
...
- Preferred: GitHub Pages (free, already wired via `.github/workflows/deploy.yml`; requires the repo to be public on the free plan; site at https://dc-guo.github.io/paintbridge/). The workflow runs tests + typecheck before deploying and only fires on pushes to `main`.
